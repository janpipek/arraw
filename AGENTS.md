...
#### Diagnostics
Set the `ARRAW_TRACE` environment variable to print per-operation timings (`[trace] <label> N ms` on stderr) for expensive work — RAW load stages, the standard image loader, and the lcms colour transforms. The facility is in `src/core/Trace.h` (`trace::Scope` for a whole scope, `trace::Laps` for multi-stage ops); it is free when the variable is unset. On Windows `arraw-gui.exe` (the editor) is a GUI-subsystem binary with no attached console, so redirect to capture it: `arraw-gui.exe 2> trace.txt`. `arraw.exe` (the console front-end) attaches a console normally.

### Tests
Use `just` or direct `ctest` execution:
* **Run tests (recommended)**: `just test` or `ctest --test-dir build --output-on-failure`
* **Test location**: Catch2 v3 tests live in `tests/`, linking the `arraw_ui` and `arraw_cli` static libraries (all source except the `main.cpp` entry points).
