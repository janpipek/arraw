...
static const FieldSpec kExposure{
    -500, 500, 0, 0.01f, 0.01f, 2, " EV", true};
...
static const FieldSpec kHue{
    -100, 100, 0, 1.0f, 0.3f, 1, QString::fromUtf8("°"), true};

// Temperature: unipolar Kelvin, no leading '+', " K" suffix.
static const FieldSpec kTemperature{
    2000, 12000, 5500, 1.0f, 1.0f, 0, " K", false};

TEST_CASE("format renders raw tick in display units with sign and suffix",
          "[fieldspec]") {
    REQUIRE(kExposure.format(150) == "+1.50 EV");
}

TEST_CASE("parse reads a display string back to a raw tick, "
          "tolerating sign, suffix and whitespace", "[fieldspec]") {
    bool ok = false;
...
TEST_CASE("parse clamps to range and rejects non-numeric input",
          "[fieldspec]") {
    bool ok = false;
...
TEST_CASE("raw <-> param round-trips through the param scale, clamped",
          "[fieldspec]") {
...
TEST_CASE("display scale is independent of param scale (Hue reads degrees)",
          "[fieldspec]") {
