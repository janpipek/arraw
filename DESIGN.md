...
- On some Linux/Wayland setups (confirmed: NVIDIA and Mesa alike, native Wayland
  and XWayland alike), the test process **SIGSEGVs on exit after every `[gpu]`
  assertion already reported success**. `valgrind` traces the fault to Qt itself —
  an invalid read in `QGuiApplicationPrivate::~QGuiApplicationPrivate()`'s
  `QThreadStorage<QFontCache*>` cleanup — triggered merely by having created a real
  GL/EGL context via `QtWaylandClient`, reproducible with a minimal Qt program
  outside this repo. It is unaffected by event-loop pumping, static destruction
  order (already correct here), or `QT_QPA_PLATFORM=offscreen`. Treat a
  `just test`/`ctest` run as green if the printed test results say so, even if the
  process then crashes on exit — this is a Qt 6.11.x/QtWayland shutdown bug, not
  an arraw regression.
